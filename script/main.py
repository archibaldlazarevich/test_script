...
def valid_date(date_str) -> datetime.date:
    """
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(f"Incorrect date format: '{date_str}'. Expected format Y-d-m (ex. - 2025-22-06).")
...
parser.add_argument(
    "-d",
    "--date",
    help='parameter for generating a report for a specified date "%Y-%m-%d" ex.: 2025-25-06',
    nargs= 1,
    type=valid_date
)
...
def create_answer_aver_dict(dict_data: dict) -> dict:
    ...
    sorted_data = dict(sorted(dict_data.items(), key=lambda x: x[1]['count'], reverse=True))
    new_dict = {
...
def create_report_with_date(date: str, file_list: list) -> dict | str:
    ...
    :return: конечные данные для вывода в консоль | сообщение о том, что данных на запрашиваемую дату нет в файлах
    """
    dict_init = dict()
    for i in file_list:
        with open(i.name) as f:
            for line in f:
                ...
                date_log = datetime.fromisoformat(result.get('@timestamp')).date()
                if date_log == date:
                    url = result.get('url')
                    if url in dict_init:
...
def args_processing(args: parser.parse_args) -> None:
    """
    ...
    """
    if args.date:
        answer = create_report_with_date(date=args.date[0], file_list=args.file)
    else:
